...
export class OllamaProvider extends Provider{
	...
	static generateSettings(containerEl: HTMLElement, plugin: AIAdapterPlugin) {
		...
		new Setting(containerEl)
			...
			.addText(text => text
				.setValue(settings.ollamaSettings.token !== '' ? '••••••••••' : '')
				.onChange(async (value) => {
					if (value.contains('•')) {
						return;
					}
	...
	async queryWithImageHandling(prompt: string, image: string): Promise<string> {
		const response: ChatResponse = await ollama.chat({
			model: settings.selectedModel.model, //llava:13b or llava or llava-llama3
	...
	private async checkOllama() {
		try {
			...
			debugLog(models);
			if (!models.models.some(model => model.name === settings.selectedModel.model)) {
				new Notice(`No ${settings.selectedModel.name} model found, please make sure you have pulled it (you can pull it over the settings tab or choose another model)`);
			}
		} catch (e) {
			debugLog(e);
	...
	static async pullImage() {
		...
		try {
			new Notice(`Pulling ${settings.selectedModel.name} model started, this may take a while...`);
			const response = await ollama.pull({model: settings.selectedModel.model, stream: true});
			progressNotice = new Notice(`Pulling ${settings.selectedModel.name} model 0%`, 0);
			for await (const part of response) {
				...
				if (part.total !== null && part.completed !== null) {
					...
					if (!isNaN(percentage) && percentage !== Infinity && percentage !== -Infinity) {
						...
						progressNotice.setMessage(`Pulling ${settings.selectedModel.name} model ${roundedNumber}% (${completed}GB/${total}GB)`);
					}
				}
			}
			progressNotice.hide();
			new Notice(`${settings.selectedModel.name} model pulled successfully`);
		} catch (e) {
			debugLog(e);
			progressNotice?.hide();
